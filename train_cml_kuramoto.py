...
def train_dynamics(args, dynamics_learner, gumbel_generator, optimizer,
                   device, train_loader, epoch, experiment,skip_conn):
...
def main():
    ...
    # Loading data
    print('Loading data')
    if args.simulation_type == 'kuramoto':
        train_loader,val_loader,test_loader,object_matrix = load_kuramoto_ggn(batch_size=128)
    elif args.simulation_type == 'cml':
        train_loader,val_loader,test_loader,object_matrix = load_cml_ggn(batch_size=128)
    else:
